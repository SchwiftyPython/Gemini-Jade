...
public class Scheduler : MonoBehaviour
{
    ...
    {
        int tempBatchSize = Mathf.Max(maxFrames, jobsPerFrame);
        DesiredNbInterations = 0;
        if (tempBatchSize > 1 && Queue.Count > tempBatchSize)
            DesiredNbInterations = Mathf.FloorToInt(Queue.Count / tempBatchSize);
        else
            DesiredNbInterations = Mathf.Min(Queue.Count, tempBatchSize);
    }
