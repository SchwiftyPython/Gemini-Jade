...
namespace Dossamer.PanZoom
{
    ...
    public class PanZoomEasingBehavior : MonoBehaviour
    {
        ...
        {
            if (UnityEngine.Time.time - lastCacheTime >= cacheInterval)
            {
                dataQueue.Enqueue(new DataCache
                {
                    panPosition = pz.transform.position,
                    panTime = UnityEngine.Time.time,
                ...
                lastCacheTime = UnityEngine.Time.time; 
            }
        }
        ...
        void StartEndTween()
        {
            ...
            if (velocity.magnitude != 0)
            {
                tweenList.Add(LeanTween.value(velocity.magnitude, 0f, tweenDuration)
                    .setOnUpdate((float value) => {
                        Vector3 translation = velocity.normalized * value * UnityEngine.Time.deltaTime;
            ...
            if (speed != 0)
            {
                tweenList.Add(LeanTween.value(speed, 0f, tweenDuration)
                    .setOnUpdate((float value) => { pzCam.orthographicSize = pz.GetClampedCameraSize(pzCam.orthographicSize + value * UnityEngine.Time.deltaTime); })
                    .setEase(tweenType)
                    .id);
            }
        }
